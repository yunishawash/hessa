...
<html direction="rtl" dir="rtl" style="direction: rtl" >
  <head>
    ...
    <!--begin::Page Custom Styles(used by this page) -->
     <link href="{{ URL::asset('/assets/css/demo6/pages/login/login-3.rtl.css') }}" rel="stylesheet" type="text/css" />
...
<!--end::Layout Skins -->
    ...
    <title>@yield('pageTitle')  | {{ __("Master.Login") }}</title>
  </head>
  ...
  <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" style="background-image: url({{ URL::asset('/assets/media/bg/bg-3.jpg')}}
);">
    <div class="kt-grid__item kt-grid__item--fluid kt-login__wrapper">
      <div class="kt-login__container">
        <div class="kt-login__logo">
          <a href="#">
            <img style="max-width: 82px;width: 140px !important;" src="{{ URL::asset('/assets/media/logos/logo-5.png')}}">
          </a>
        </div>
        <div class="kt-login__signin">
          <div class="kt-login__head">
            <h3 class="kt-login__title">{{ __("Master.SignIn") }}</h3>
          </div>

          <form class="kt-form" action='/auth/login'  method="post"  >
             @csrf
            <div class="row">
              <div class="form-group col-12">
                <input class="form-control" type="text" title="{{ __("Master.user_email_placeholder") }}" placeholder="{{ __("Master.Email_Placeholder") }}" name="email" autocomplete="off">
                 {{ $errors->has('email')?$errors->first('email'): null }}
              </div>
            </div>
            <div class="row">
              <div class="form-group col-12">
                <input class="form-control" type="password" title="{{ __("Master.password_title") }}" placeholder="{{ __("Master.Password_Placeholder") }}" name="password">
                {{ $errors->has('email')?$errors->first('password'): null }}
              </div>
            </div>
            <div class="row kt-login__extra">
              <div class="col">
                <label class="kt-checkbox">
                  <input type="checkbox" value="true" name="remember_me"> {{ __("Master.Remember_me") }}
                  <span></span>
                </label>
              </div>
              <div class="col kt-align-right">
                <a href="javascript:;" id="kt_login_forgot" class="kt-login__link">{{ __("Master.Forget_Password") }}</a>
              </div>
            </div>
            <div class="kt-login__actions">
              <button id="kt_login_signin_submit"  type='submit' class="btn btn-brand btn-elevate kt-login__btn-primary">{{ __("Master.SignIn") }}</button>
            </div>
          </form>
        </div>
        <div class="kt-login__signup">
          <div class="kt-login__head">
            <h3 class="kt-login__title">{{ __("Master.sp_SignUp") }}</h3>
          </div>
          <form class="kt-form" method="post" enctype="multipart/form-data" accept-charset="UTF-8"  action="/serviceProvider/store">
            <!--begin::Accordion-->
                ...
                <!--end::Accordion-->
            <div class="kt-login__actions">
              <button id="kt_login_signup_submit" class="btn btn-brand btn-elevate kt-login__btn-primary">{{ __("Master.SignUp") }}</button>&nbsp;&nbsp;
              <button id="kt_login_signup_cancel" class="btn btn-light btn-elevate kt-login__btn-secondary">{{ __("Master.Cancel") }}</button>
            </div>
          </form>
        </div>
        <div class="kt-login__forgot">
          <div class="kt-login__head">
            <h3 class="kt-login__title">{{ __("Master.Forgotten_Password") }} </h3>
            <div class="kt-login__desc">{{ __("Master.Enter_your_email") }} </div>
          </div>
          <form class="kt-form"  method="post" action="/auth/ForgetPassword">
            <div class="input-group">
              <input class="form-control" type="email" title="{{ __("Master.user_email_placeholder") }}" placeholder="{{ __("Master.Email_Placeholder") }}"  name="email" id="kt_email" autocomplete="off" required="">
            </div>
            <div class="kt-login__actions">
              <button id="kt_login_forgot_submit" class="btn btn-brand btn-elevate kt-login__btn-primary">{{ __("Master.Request") }}</button>&nbsp;&nbsp;
              <button id="kt_login_forgot_cancel" class="btn btn-light btn-elevate kt-login__btn-secondary">{{ __("Master.Cancel") }}</button>
            </div>
          </form>
        </div>
        <div class="kt-login__account">
          <span class="kt-login__account-msg">
            {{ __("Master.Sign_Up_msg") }}
          </span>
          <u ><a href="javascript:;" style='color:blue;' id="kt_login_signup" class="kt-login__account-link">{{ __("Master.Sign_Up") }} </a></u>
        </div>
      </div>
    </div>
  </div>
...
<!--begin::Page Scripts(used by this page) -->
    <script src="{{ URL::asset('/assets/js/demo6/pages/login/login-general.js') }}" type="text/javascript"></script>
<!--end::Page Scripts -->
<style type="text/css">
  @media (min-width: 1200px) {
        .kt-login__signup, .kt-login__container{
          width: 1200px !important;
        }
      }
      @media (max-width: 430px) {
        .kt-login__signup, .kt-login__forgot{
          width: 100% !important;
        }
      }
    @media (min-width: 430px) {
      .kt-login__signin, .kt-login__forgot{
